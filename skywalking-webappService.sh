...
echo "Skywalking Web Application run at ${SKYWALKING_HOST}:${SKYWALKING_WEBAPP_PORT}"
eval exec "\"$_RUNJAVA\" ${JAVA_OPTS} -jar ${JAR_PATH}/skywalking-webapp.jar \
         --server.port=${SKYWALKING_WEBAPP_PORT} --collector.ribbon.listOfServers=${SKYWALKING_HOST}:9921 \
